...
export function RunsView({ showHeader = true }: { showHeader?: boolean }) {
    ...
    return (
        <div className={showHeader ? 'container mx-auto p-6 space-y-6' : 'space-y-6'}>
            <div>
                <div className={showHeader ? 'flex justify-between items-start' : 'flex justify-end items-start'}>
                    {showHeader && (
                    <div>
                        <p className="text-eyebrow text-primary">Analytics</p>
                        <h1 className="text-h1 mt-1">Agent Runs</h1>
                        <p className="text-body mt-1 text-muted-foreground">See all your Agent Runs across all Voice Agents. You can use filters to filter out required Agent Runs.</p>
                    </div>
                    )}
